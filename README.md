...
- `SHIPPER_REPO_TOKENS` a json object, that holds the authentication token for remote repositories such as [Zenodo](https://zenodo.org/), required for shipping to Zenodo (optional). Can have the following form `{"zenodo": "$ZENODO_TOKEN", "zenodo_sandbox": "$ZENODO_SANDBOX_TOKEN", "download": "" }`. Replace `$ZENODO_TOKEN` etc. with your personal access token.
